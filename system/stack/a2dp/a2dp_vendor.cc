...
bool A2DP_IsVendorSourceCodecValid(const uint8_t* p_codec_info) {
  ...
  if (vendor_id == A2DP_LHDC_VENDOR_ID && codec_id == A2DP_LHDC_CODEC_ID) {
    return A2DP_IsCodecValidLhdc(p_codec_info);
  }
...
tA2DP_STATUS A2DP_IsVendorSinkCodecSupported(const uint8_t* p_codec_info) {
  ...
  // Check for LHDC
  if (vendor_id == A2DP_LHDC_VENDOR_ID && codec_id == A2DP_LHDC_CODEC_ID) {
    return A2DP_IsVendorSinkCodecSupportedLhdc(p_codec_info);
  }
...
bool A2DP_VendorUsesRtpHeader(bool content_protection_enabled, const uint8_t* p_codec_info) {
  ...
  // Check for LHDC
  if (vendor_id == A2DP_LHDC_VENDOR_ID && codec_id == A2DP_LHDC_CODEC_ID) {
    return A2DP_VendorUsesRtpHeaderLhdc(content_protection_enabled, p_codec_info);
  }
...
bool A2DP_VendorCodecTypeEquals(const uint8_t* p_codec_info_a, const uint8_t* p_codec_info_b) {
  ...
  // Check for LHDC V5
  if (vendor_id_a == A2DP_LHDCV5_VENDOR_ID && codec_id_a == A2DP_LHDCV5_CODEC_ID) {
...
int A2DP_VendorGetBitRate(const uint8_t* p_codec_info) {
  ...
  // Check for LHDC
  if (vendor_id == A2DP_LHDC_VENDOR_ID && codec_id == A2DP_LHDC_CODEC_ID) {
    return A2DP_VendorGetBitRateLhdc(p_codec_info);
  }
...
bool A2DP_VendorBuildCodecHeader(const uint8_t* p_codec_info, BT_HDR* p_buf,
  ...
  // Check for LHDC
  if (vendor_id == A2DP_LHDC_VENDOR_ID && codec_id == A2DP_LHDC_CODEC_ID) {
    return A2DP_VendorBuildCodecHeaderLhdc(p_codec_info, p_buf, frames_per_packet);
  }
...
std::string A2DP_VendorCodecInfoString(const uint8_t* p_codec_info) {
  ...
  // Check for LHDC
  if (vendor_id == A2DP_LHDC_VENDOR_ID && codec_id == A2DP_LHDC_CODEC_ID) {
    return A2DP_VendorCodecInfoStringLhdc(p_codec_info);
  }
